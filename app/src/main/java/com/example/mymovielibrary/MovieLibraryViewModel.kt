...
class MovieLibraryViewModel : ViewModel() {
    ...
    private suspend fun getAllMovies() = withContext(Dispatchers.IO) {
        MovieApi.retrofitService.getProperties("65db5aebb7dc29d77c7b00443904e829").enqueue(object : Callback<MovieProperties> {
            override fun onFailure(call: Call<MovieProperties>, t: Throwable) {
                _serverResponse.postValue("Failure: " + t.message)
            }

            override fun onResponse(call: Call<MovieProperties>, response: Response<MovieProperties>) {
